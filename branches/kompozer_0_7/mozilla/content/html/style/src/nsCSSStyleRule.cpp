...
#include "nsIDOMNSCSSSelectorQuery.h"
...
void
nsAtomList::ToDOMStringList(const nsAString & aPrefix, nsIDOMDOMStringList * aStringList)
{
  if (nsnull != mAtom) {
    nsAutoString atomString, classString(aPrefix);
    mAtom->ToString(atomString);
    classString.Append(atomString);
    nsCOMPtr<nsDOMStringList> list = do_QueryInterface(aStringList);
    list->Add(classString);
  }
  if (nsnull != mNext) {
    mNext->ToDOMStringList(aPrefix, aStringList);
  }
}
...
void
nsCSSSelector::GetSelectorList(PRUint32 aSelectorFilter, nsIDOMDOMStringList * aStringList, nsICSSStyleSheet* aSheet)
{
  if ((aSelectorFilter & nsIDOMNSCSSSelectorQuery::SHOW_TYPE_SELECTOR) &&
      mTag && !IsPseudoElement(mTag))
  {
  ...
  if ((aSelectorFilter & nsIDOMNSCSSSelectorQuery::SHOW_CLASS_SELECTOR) && mClassList) {
    mClassList->ToDOMStringList(NS_LITERAL_STRING("."), aStringList);
  }
  if ((aSelectorFilter & nsIDOMNSCSSSelectorQuery::SHOW_ID_SELECTOR) && mIDList) {
    mIDList->ToDOMStringList(NS_LITERAL_STRING("#"), aStringList);
  }
  if (mNegations) {
    mNegations->GetSelectorList(aSelectorFilter, aStringList, aSheet);
  }
  if (mNext) {
    mNext->GetSelectorList(aSelectorFilter, aStringList, aSheet);
  }
}
...
class DOMCSSStyleRuleImpl : public nsICSSStyleRuleDOMWrapper,
                            ...
                            public nsIDOMNSCSSSelectorQuery
{
public:
  ...
  NS_DECL_NSIDOMNSCSSSELECTORQUERY
...
NS_INTERFACE_MAP_BEGIN(DOMCSSStyleRuleImpl)
  ...
  NS_INTERFACE_MAP_ENTRY(nsIDOMNSCSSSelectorQuery)
  NS_INTERFACE_MAP_ENTRY(nsIDOMCSSStyleRule)
  NS_INTERFACE_MAP_ENTRY(nsIDOMCSSRule)
  NS_INTERFACE_MAP_ENTRY_AMBIGUOUS(nsISupports, nsIDOMCSSRule)
...
DOMCSSStyleRuleImpl::GetSelectorList(PRUint32 aSelectorFilter, nsIDOMDOMStringList ** aStringList)
{
  ...
  nsCOMPtr<nsDOMStringList> list = new nsDOMStringList();
  if (!list) {
    *aStringList = nsnull;
    return NS_ERROR_OUT_OF_MEMORY;
  }

  nsresult res = Rule()->GetSelectorList(aSelectorFilter, list);
  if (NS_FAILED(res))
    return res;
...
public:
  ...
  virtual nsresult GetSelectorList(PRUint32 aSelectorFilter, nsIDOMDOMStringList * aStringList);
...
CSSStyleRuleImpl::GetSelectorList(PRUint32 aSelectorFilter, nsIDOMDOMStringList * aStringList)
{
  if (mSelector)
  {
    ...
    while (selectorList)
    {
      selectorList->mSelectors->GetSelectorList(aSelectorFilter, aStringList, mSheet);
