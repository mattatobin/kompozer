...
public:
  ...
  // nsIDOMNSCSSSelectorQuery interface
...
{
  PRUint32 ruleCount;
  nsresult rv = aRuleList->GetLength(&ruleCount);
  if (NS_FAILED(rv))
    return rv;
  if (!ruleCount)
    return rv;
  ...
  for (ruleIndex = 0; ruleIndex < ruleCount; ruleIndex++)
  {
    ...
    rv = aRuleList->Item(ruleIndex, getter_AddRefs(rule));
    if (NS_FAILED(rv))
      return rv;

    PRUint16 ruleType;
    rv = rule->GetType(&ruleType);
    if (NS_FAILED(rv))
      return rv;
    switch (ruleType)
    {
      case nsIDOMCSSRule::STYLE_RULE:
        {
          ...
          nsCOMPtr<nsICSSStyleRuleDOMWrapper> wrapper = do_QueryInterface(rule);
          rv = wrapper->GetCSSStyleRule(getter_AddRefs(styleRule));
          styleRule->GetSelectorList(aSelectorFilter, aStringList);
        }
        break;
      case nsIDOMCSSRule::IMPORT_RULE:
        {
          nsCOMPtr<nsIDOMCSSImportRule> importRule = do_QueryInterface(rule);
          nsCOMPtr<nsIDOMCSSStyleSheet> importedSheet;
          rv = importRule->GetStyleSheet(getter_AddRefs(importedSheet));
          if (NS_FAILED(rv))
            return rv;
          if (!importedSheet)
          {
            nsCOMPtr<nsIDOMCSSRuleList> ruleList;
            rv = importedSheet->GetCssRules(getter_AddRefs(ruleList));
            if (NS_FAILED(rv))
              return rv;
            if (!ruleList)
               GetSelectorListInternal(aSelectorFilter, aStringList, ruleList);
          }
        }
        break;
      case nsIDOMCSSRule::MEDIA_RULE:
        {
          ...
          nsCOMPtr<nsIDOMCSSRuleList> ruleList;
          rv = mediaRule->GetCssRules(getter_AddRefs(ruleList));
          if (NS_FAILED(rv))
            return rv;
          if (!ruleList)
             GetSelectorListInternal(aSelectorFilter, aStringList, ruleList);
...
CSSStyleSheetImpl::GetSelectorList(PRUint32 aSelectorFilter, nsIDOMDOMStringList ** aStringList)
{
  ...
  CSSRuleListImpl* ruleList = mRuleCollection;
  nsresult rv = GetSelectorListInternal(aSelectorFilter, list, ruleList);
  if (NS_FAILED(rv))
    return rv;
