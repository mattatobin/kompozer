...
nsresult nsCopySupport::HTMLCopy(nsISelection *aSel, nsIDocument *aDoc, PRInt16 aClipboardID)
{
  ...
  if (bIsHTMLCopy)
  {
    ...
    flags = nsIDocumentEncoder::PreserveNvuElements;
#endif
  }
  else
  {
    ...
    if (htmlDoc)
    {
      ...
      rv = htmlDoc->GetBody(getter_AddRefs(bodyElem));
      if (NS_SUCCEEDED(rv) && bodyElem)
      {
        nsAutoString sourceViewVal;
        rv = bodyElem->GetAttribute(NS_LITERAL_STRING("_moz_sourceview"), sourceViewVal);
        if (NS_SUCCEEDED(rv) && sourceViewVal.Equals(NS_LITERAL_STRING("true")))
        {
          flags |= nsIDocumentEncoder::OutputRaw | nsIDocumentEncoder::OutputLineBreaksWhenClosingLI;
          mimeType = NS_LITERAL_STRING(kTextMime);
        }
      }
    }
#endif
