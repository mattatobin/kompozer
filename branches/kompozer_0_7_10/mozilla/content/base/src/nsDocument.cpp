...
NS_IMETHODIMP
nsDocument::GetSelectorList(PRUint32 aSelectorFilter, nsIDOMDOMStringList ** aStringList)
{
  ...
  for (sheetIndex = 0; sheetIndex < sheetCount; sheetIndex++)
  {
    ...
    nsCOMPtr<nsIDOMCSSStyleSheet> domCSSSheet(do_QueryInterface(sheet));
    if (domCSSSheet)
    {
      nsCOMPtr<nsIDOMCSSRuleList> ruleList;
      domCSSSheet->GetCssRules(getter_AddRefs(ruleList));
      if (ruleList)
      {
        nsCOMPtr<nsICSSStyleSheet> cssSheet(do_QueryInterface(sheet));
        rv = cssSheet->GetSelectorListInternal(aSelectorFilter,
                                               list,
                                               ruleList);
        if (NS_FAILED(rv))
          return rv;
