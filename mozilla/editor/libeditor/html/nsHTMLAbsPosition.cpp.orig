...
nsHTMLEditor::SetFinalPosition(PRInt32 aX, PRInt32 aY)
{
  ...
  mHTMLCSSUtils->RemoveCSSProperty(mAbsolutelyPositionedObject,
                                   nsEditProperty::cssBottom,
                                   emptyStr, PR_FALSE);
  mHTMLCSSUtils->RemoveCSSProperty(mAbsolutelyPositionedObject,
                                   nsEditProperty::cssRight,
                                   emptyStr, PR_FALSE);
                                        ...
                                        PRBool aEnabled)
{
  ...
  if (aEnabled) {
    ...
    nsCOMPtr<nsIDOMNode> parentNode;
    PRInt32 offset;
    res = GetNodeLocation(aElement, address_of(parentNode), &offset);
    if (NS_FAILED(res)) return res;
    nsCOMPtr<nsIDOMNode> brNode;
    res = CreateBR(parentNode, offset, address_of(brNode));
    if (NS_FAILED(res)) return res;
