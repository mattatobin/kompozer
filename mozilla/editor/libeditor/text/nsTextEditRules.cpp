...
nsTextEditRules::IsBogus()
{
  ...
  nsCOMPtr<nsIDOMElement> theBody;
  nsresult res = mEditor->GetRootElement(getter_AddRefs(theBody));
  if (NS_FAILED(res)) return res;
  if (!theBody) return NS_ERROR_FAILURE;
  nsCOMPtr<nsIDOMNode> node = mEditor->GetLeftmostChild(theBody);
  if (node && node != mBogusNode)
  {
    mEditor->DeleteNode(mBogusNode);
    mBogusNode = do_QueryInterface(nsnull);
    return PR_FALSE;
  ...
  res = mBogusNode->GetNextSibling(getter_AddRefs(nextNode));
  if (NS_SUCCEEDED(res) && nextNode)
  {
    mEditor->DeleteNode(mBogusNode);
    mBogusNode = do_QueryInterface(nsnull);
    return PR_FALSE;
